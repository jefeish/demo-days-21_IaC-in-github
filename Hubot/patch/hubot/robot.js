...
class Robot {
    ...
    constructor(adapterPath, adapter, httpd, name, alias) {
        ...
        if (process.env.HUBOT_LOG_FILE) {
            const stream = fs.createWriteStream(process.env.HUBOT_LOG_FILE, {flags:'a'})
            this.logger = new Log(process.env.HUBOT_LOG_LEVEL || 'info', stream)
        }
        else {
